...
def calculate_index(height, weight):
    ...
    if bmi <= 18.5:
        messagebox.showinfo('bmi-pythonguides',
                            f'ИМТ = {bmi} соответствует недостаточному весу')
    elif (bmi > 18.5) and (bmi <= 24.9):
        messagebox.showinfo('bmi-pythonguides',
                            f'ИМТ = {bmi} соответствует нормальному весу')
    elif (bmi > 24.9) and (bmi <= 29.9):
        messagebox.showinfo('bmi-pythonguides',
                            f'ИМТ = {bmi} соответствует избыточному весу')
    else:
        messagebox.showinfo('bmi-pythonguides',
                            f'ИМТ = {bmi} соответствует ожирению')
...
disc_message = tk.Label(root, text="Введите свои данные", font=("Arial", 24),
                        bg="#1e1e1e", fg="white", bd=0, anchor="center")
...
weigth = tk.Entry(frame, validate="key",
...
height = tk.Entry(frame, validate="key",
                  validatecommand=(validate_digit_command, "%P"))
height.grid(row=2, column=1, pady=10)
height.configure(font=20)

# Кнопка подсчета
calc_index = tk.Button(frame, text="Подсчитать",
                       command=lambda: calculate_index(int(height.get()),
                                                       int(weigth.get())))
